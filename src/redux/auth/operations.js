...
export const logout = createAsyncThunk('auth/logout',
    async (_, thunkAPI) => {
        try {
            const res = await axios.post('https://connections-api.herokuapp.com/users/logout')
            clearAuthHeader()
            return res.data
        } catch (err) {
            console.log(err)
...
export const refreshUser = createAsyncThunk('auth/refresh',
    async (_, thunkAPI) => {
        ...
        try {
            ...
            const res = await axios.get('https://connections-api.herokuapp.com/users/current')
            return res.data
        } catch (err) {
            console.log(err)
            return thunkAPI.rejectWithValue(err.message)
